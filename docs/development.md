...
- **交互式音频波形可视化重构 (2026-04-21)**：
  - **架构解耦**：引入 `wavesurfer.js`，新建 `WaveformPlayer.vue` 独立组件。彻底废弃 `Detail.vue` 中的原生 `new Audio()` 对象及手动进度同步逻辑。
  - **单栏布局进化**：应业务需求移除右侧 Tab 控制面板，将聊天流区域扩展为全宽布局（内置 10% 黄金间距），大幅提升阅读体验。
  - **底部悬浮控制舱**：设计并实现了一个基于 `backdrop-blur` 效果的底部悬浮控制台。集成了波形展示、文本实时同步、角色勋章及播放控制功能。
  - **视觉语言对标 ElevenLabs**：波形颜色采用暖灰（`#e5e5e5`）搭配纯黑进度（`#000000`），去除所有 Element UI 滑块，实现像素级的极简主义响应式交互。
  - **性能与内存管理**：在组件销毁期严格执行 `wavesurfer.destroy()`，确保单实例覆盖加载模式下的内存安全。

- **对话纠错工具链集成与 UI 细节打磨 (2026-04-21)**:
  - **工业级纠错工具组 (User Correction Suite)**：
    - **说话人实时切换 (Switch Speaker)**：在对话气泡元数据区集成 `updateSegmentSpeaker` 接口。采用 `Switch` 交互图标，支持 `spk0/spk1` 角色的一键对调，并实现前端数据状态的响应式回显。
    - **对话物理切分 (Dialogue Split)**：基于 `wavesurfer.js` 的光标定位能力，通过 `getCurrentTime()` 导出毫秒级时间戳，配合 `Scissor`（剪刀）图标触发后端“免 OOM 二次转录”流程。
  - **音频渲染与播放体验深度优化**：
    - **极致波形条设计**：重构 `WaveformPlayer.vue` 参数，将波形宽度设为 `barWidth: 1`，间距设为 `barGap: 3`，模拟专业音频工作站（DAW）的高频采样视觉质感，色彩对标 ElevenLabs 纯黑美学。
    - **多级倍速控制**：底部控制舱新增 `0.5x - 2.0x` 动态倍速调节滑块，支持用户根据方言辨识难度自主调整听读节奏。
  - **系统健壮性与 Bug 修复**：
    - **图标库引用纠偏**：修复了 `Detail.vue` 中 `Scissors` (复数) 与 Element Plus 官方导出项 `Scissor` (单数) 的命名冲突，解决了导致页面白屏的 `SyntaxError`。
    - **API 服务层对齐**：封装了 `splitSegment` 等支持 120s 超长等待的后端重推理 API。
